...
@section('content')
  ...
  <div class="container-xxl flex-grow-1 container-p-y">
    <a href="/admin/soal-lanjutan" class="btn btn-outline-primary mb-3">
      <span class="tf-icons bx bx-left-arrow-alt"></span>&nbsp; Kembali
    </a>
    <div class="card">
      <div class="card-header d-flex align-items-center justify-content-between">
        <h5 class="mb-0">List Data {{ $title }} Pendidikan {{ $pendidikan->nama_pendidikan }}</h5>
        <a href="/admin/soal-lanjutan/{{ $id_pendidikan_instansi }}/tambah-soal/create" class="btn btn-primary">
          <span class="tf-icons bx bx-plus"></span>&nbsp; Tambah Soal
        </a>
      </div>
    ...
    <script type="text/javascript">
      ...
      $(document).ready(function() {
        table = $('#{{ $table_id }}').DataTable({
          ...
          serverSide: true,
          ajax: {
            url: '{{ url('admin/getSoalLanjutan') }}/' + {{ $id_pendidikan_instansi }},
            type: "POST",
      ...
      function deleteData(id) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: "{{ url('/admin/soal-lanjutan/' . $id_pendidikan_instansi . '/tambah-soal') }}/" + id,
              data: {
                _method: "DELETE",
                _token: "{{ csrf_token() }}"
              },
