...
@section('content')
  <div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="card mb-4">
      <h5 class="card-header">Tambah Soal Lanjutan Pendidikan {{ $pendidikan->nama_instansi }}</h5>
      <form class="card-body" action="/admin/soal-lanjutan/{{ $id_pendidikan_instansi }}/tambah-soal" method="post"
        enctype="multipart/form-data">
        @csrf
        <div class="form-group mb-3">
          <label for="soal">Soal</label>
          <textarea name="soal" id="soal" cols="5" rows="5"
            class="form-control @error('soal') is-invalid @enderror" required>{!! old('soal') !!}</textarea>
          @error('soal')
          ...
          <div class="col-md-6 mb-2">
            <div class="form-group">
              ...
              <textarea name="jawaban_a" id="jawaban_a" cols="5" rows="5"
                class="form-control @error('jawaban_a') is-invalid @enderror" required>{!! old('jawaban_a') !!}</textarea>
              @error('jawaban_a')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
          ...
          <div class="col-md-6 mb-2">
            <div class="form-group">
              ...
              <textarea name="jawaban_b" id="jawaban_b" cols="5" rows="5"
                class="form-control @error('jawaban_b') is-invalid @enderror" required>{!! old('jawaban_b') !!}</textarea>
              @error('jawaban_b')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
          ...
          <div class="col-md-6 mb-2">
            <div class="form-group">
              ...
              <textarea name="jawaban_c" id="jawaban_c" cols="5" rows="5"
                class="form-control @error('jawaban_c') is-invalid @enderror" required>{!! old('jawaban_c') !!}</textarea>
              @error('jawaban_c')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
          ...
          <div class="col-md-6 mb-2">
            <div class="form-group">
              ...
              <textarea name="jawaban_d" id="jawaban_d" cols="5" rows="5"
                class="form-control @error('jawaban_d') is-invalid @enderror" required>{!! old('jawaban_d') !!}</textarea>
              @error('jawaban_d')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
          ...
          <div class="col">
            <div class="form-group">
              ...
              <select class="form-select @error('kunci_jawaban') is-invalid @enderror" name="kunci_jawaban"
                ...
                <option value="A" {{ old('kunci_jawaban') === 'A' ? 'selected' : '' }}>A
                </option>
                <option value="B" {{ old('kunci_jawaban') === 'B' ? 'selected' : '' }}>B
                </option>
                <option value="C" {{ old('kunci_jawaban') === 'C' ? 'selected' : '' }}>C
                </option>
                <option value="D" {{ old('kunci_jawaban') === 'D' ? 'selected' : '' }}>D
                </option>
              </select>
              @error('kunci_jawaban')
                <div class="invalid-feedback">{{ $message }}</div>
